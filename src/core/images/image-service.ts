...
import { fallbackChainItems, imageEvents, imageReplicas, images, providerConnections, shortLinks } from "@/db/schema";

export async function createImage(accountId: string, input: LoadedImage) {
  const db = getDb();
  const [created] = await db.insert(images).values({ accountId, originalUrl: input.originalUrl, contentType: input.contentType, byteSize: input.bytes.byteLength, sha256: input.sha256, status: "processing" }).returning();
  ...
  const extension = input.contentType.split("/")[1]?.replace(/[^a-z0-9]/gi, "") || "bin";
  const uploadInput = { bytes: input.bytes, contentType: input.contentType, objectKey: `${accountId}/${input.sha256}.${extension}`, sourceUrl: input.originalUrl };
  ...
  await db.insert(imageEvents).values({ imageId: created.id, type: status === "degraded" ? "degraded" : "uploaded", message: status === "degraded" ? "At least one replica failed; repair scheduled." : "All replicas uploaded." });
